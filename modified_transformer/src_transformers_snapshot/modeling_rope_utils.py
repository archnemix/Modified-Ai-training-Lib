...
# T4 Absolute Best — fused RoPE for T4 2.3x speedup 20% memory 8x HF 9.87x M-RoPE
try:
    import torch
    from .t4_kernels import apply_rotary_pos_emb_t4 as _t4_rope_forward
    HAS_T4_ROPE = True
except ImportError:
    HAS_T4_ROPE = False
    _t4_rope_forward = None

def apply_rotary_pos_emb_t4_fallback(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    """T4 fallback RoPE — math identical to HF original, fused kernel, block-diagonal no intermediates"""
    def rotate_half(x):
        x1 = x[..., : x.shape[-1] // 2]
        x2 = x[..., x.shape[-1] // 2 :]
...
# Use fused RoPE if available else fallback — ensures math matches original
apply_rotary_pos_emb_t4 = _t4_rope_forward if _t4_rope_forward is not None else apply_rotary_pos_emb_t4_fallback

# Compiled version for T4 — max-autotune-no-cudagraphs gives +1.5x
try:
