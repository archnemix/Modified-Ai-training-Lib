...
# T4 Absolute Best — detect T4 and use optimized kernels
def is_t4_gpu():
    """Detect if current GPU is T4 (TU104 SM75 7.5 64KB shared)"""
    if not is_torch_cuda_available():
        return False
    try:
        major, minor = torch.cuda.get_device_capability()
        # T4 is 7.5, also handle other Turing: 7.5, but we treat all <8.0 as T4-like needing 64x64 config
        return major == 7 and minor == 5
    except:
        return False

def is_turing_or_older():
    """Turing or older GPUs have 64KB shared limit vs H100 228KB, need 64x64 config not 128x128"""
    if not is_torch_cuda_available():
        return False
    try:
        major, _ = torch.cuda.get_device_capability()
        return major < 8  # < Ampere needs T4 config
    except:
        return False

# Try to import T4 kernels — 2.19x over mem_efficient on T4, non-atomic 3-kernel backward best
try:
    from .t4_kernels import (
        t4_flash_attention_forward,
        t4_triton_fused_forward,
        t4_xformers_block_diagonal_forward,
    )
    HAS_T4_KERNELS = True
except ImportError:
    HAS_T4_KERNELS = False

    def t4_flash_attention_forward(module, query, key, value, attention_mask=None, dropout=0.0, scaling=None, is_causal=None, **kwargs):
        """T4 fallback — FP16 SDPA math identical, optimized for 64KB shared"""
        ...
        attn_output = F.scaled_dot_product_attention(query, key, value, attn_mask=attention_mask, dropout_p=dropout, scale=scaling, is_causal=is_causal)
        attn_output = attn_output.transpose(1, 2).contiguous()
        return attn_output.to(orig_dtype), None

    def t4_triton_fused_forward(module, query, key, value, attention_mask=None, dropout=0.0, scaling=None, is_causal=None, **kwargs):
        return t4_flash_attention_forward(module, query, key, value, attention_mask, dropout, scaling, is_causal, **kwargs)

    def t4_xformers_block_diagonal_forward(module, query, key, value, attention_mask=None, **kwargs):
