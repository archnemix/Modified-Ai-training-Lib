...
# T4 Absolute Best — fused kernels for RMSNorm, RoPE, SwiGLU, CE
try:
    from .t4_kernels import (
        FusedRMSNormT4,
        FusedSwiGLUT4,
        FusedCrossEntropyT4,
        apply_rotary_pos_emb_t4,
        T4_CONFIG,
    )
    HAS_T4_FUSED = True
except ImportError:
    HAS_T4_FUSED = False
    T4_CONFIG = {"BLOCK_M": 64, "BLOCK_N": 64, "BLOCK_K": 32, "num_warps": 4, "num_stages": 1}
...
# T4 compiled versions — torch.compile max-autotune-no-cudagraphs dynamic=False gives +1.5x MLP, +20% overall
try:
...
# Alias for models that want T4 absolute best — math identical to HF original, just faster
